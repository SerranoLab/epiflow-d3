...
const App = {
  ...
  // R10: the API's EPIFLOW_VERSION is the single source; fill the badge, the
  // About line and the page footer from it, and keep it for the report.
  version: '',
  async loadVersion() {
    try {
      const h = await EpiFlowAPI.health();
      this.version = String(h.version || '');
    } catch (e) { this.version = ''; }
    document.querySelectorAll('.app-version').forEach(el => { el.textContent = this.version ? 'v' + this.version : ''; });
    console.log('EpiFlow D3 ' + (this.version ? 'v' + this.version : '(version unknown)') + ' initialized');
  },
  ...
  applyCustomColors() {
    const container = document.getElementById('custom-color-pickers');
    const varName = (container && container.dataset.var) || 'genotype';
    const pickers = document.querySelectorAll('#custom-color-pickers input[type="color"]');
    const customPal = {};
    pickers.forEach(p => { customPal[p.dataset.level] = p.value; });
    if (!DataManager.serverPalette) DataManager.serverPalette = {};
    DataManager.serverPalette[varName] = customPal;
    this.loadCurrentTab();
  },
  ...
  async runAllMarkers() {
    ...
    try {
      ...
      // Display caution notes (LMM low-replicate warning)
  ...
  async runGroupedCV() {
    ...
    this.showLoading('Grouped sample-level CV (' + method.toUpperCase() + ')...');
  ...
  renderDiagnosticGroupedCv(containerId, cv) {
    ...
    // L11: the title follows cv_type — leave-one-sample-out up to 10 samples, grouped 5-fold above.
    const cvType = (cv && cv.cv_type) ? cv.cv_type : 'sample-level';
    const head = `<h4 style="margin:8px 0 4px;"><i class="fas fa-vials"></i> Diagnostic accuracy — grouped CV, ${cvType} (LDA)</h4>`;
  ...
  renderStratifiedGroupedCv(st) {
    ...
    let html = `<h4 style="margin:14px 0 4px;"><i class="fas fa-layer-group"></i> Per-stratum grouped CV — by ${by} (split type per row: leave-one-sample-out up to 10 samples, grouped 5-fold above)</h4>`;
    if (st.error) {
    ...
    rows.forEach(r => {
      ...
      html += `<tr><td><strong>${stratum}</strong></td><td>${Number(r.n_cells).toLocaleString()}</td>
        ...
        <td style="font-size:11px;color:#15803d;">replicate-level ✓${r.cv_type ? ' · ' + r.cv_type : ''}</td></tr>`;
    });
    html += '</tbody></table>';
  ...
  _buildReport() {
    ...
    sections.push(`
      <div class="report-section methods">
        ...
        <p style="font-size:10px;color:#94a3b8;">EpiFlow D3 ${this.version ? 'v' + this.version : ''} · © 2025–2026 Serrano Lab, CReM, Boston University · AGPL-3.0 · Generated ${timestamp}</p>
      </div>
    `);
